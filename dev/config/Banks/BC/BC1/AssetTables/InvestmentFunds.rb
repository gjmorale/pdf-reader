class BC1::InvestmentFundsCLP < BC1::AssetTable
	def load
		@name = "fondos de inversion en clp"
		@title = Field.new("Fondos de Inversión en Pesos Chilenos: Vigentes al")
		@table_end = Field.new("Monto Total Vigente en Pesos Chilenos:")
		@headers = []
			headers << HeaderField.new("Fondo", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Serie", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Características", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Cuenta", headers.size, Custom::FIN_RUT, false)
			headers << HeaderField.new("Nº Cuotas", headers.size, Setup::Type::AMOUNT, false)
			headers << HeaderField.new("Valor Cuota ($)", headers.size, Setup::Type::AMOUNT, false)
			headers << HeaderField.new("Monto ($)", headers.size, Setup::Type::AMOUNT, true)
		@total = SingleField.new("Monto Total Vigente en Pesos Chilenos:",
			[Setup::Type::AMOUNT], 
			3, Setup::Align::LEFT)
		@page_end = 		Field.new("Página ")
		@price_index = 		5
		@quantity_index = 	4
		@value_index = 		6
		@total_index = 		0
	end
end

class BC1::InvestmentFundsUSD < BC1::AssetTable
	def load
		@name = "fondos de inversion en usd"
		@title = Field.new("Fondos de Inversión en Dólares: Vigentes al")
		@table_end = Field.new("Monto Total Vigente en Dólares:")
		@headers = []
			headers << HeaderField.new("Fondo", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Serie", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Características", headers.size, Setup::Type::LABEL, false)
			headers << HeaderField.new("Cuenta", headers.size, Custom::FIN_RUT, false)
			headers << HeaderField.new("Nº Cuotas", headers.size, Setup::Type::AMOUNT, false)
			headers << HeaderField.new("Valor Cuota (US$)", headers.size, Setup::Type::AMOUNT, false)
			headers << HeaderField.new("Monto (US$)", headers.size, Setup::Type::AMOUNT, true)
		@total = SingleField.new("Monto Total Vigente en Dólares:",
			[Setup::Type::AMOUNT], 
			3, Setup::Align::LEFT)
		@page_end = 		Field.new("Página ")
		@price_index = 		5
		@quantity_index = 	4
		@value_index = 		6
		@total_index = 		0
		@alt_currency = 	:usd
	end
end