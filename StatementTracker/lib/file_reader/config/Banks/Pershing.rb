...
PER.class_eval do
	...
	private  
		...
		def analyse_position_eng factory
			set_date Institution::MONTHS, @reader.find_text(/[A-Z][a-z]{3,9}\s\d\d?,\s20\d\d - [A-Z][a-z]{3,9}\s\d\d?,\s20\d\d/)
		...
		def analyse_position_esp factory
			set_date Institution::MESES, @reader.find_text(/[A-Z][a-z]{3,9}\s\d\d?,\s20\d\d - [A-Z][a-z]{3,9}\s\d\d?,\s20\d\d/)
