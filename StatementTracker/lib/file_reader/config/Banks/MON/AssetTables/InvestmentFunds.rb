class MON::InvestmentFundsCLP < MON::AssetTable
	def load
		@name = "fondos de inversion clp"
		@title = Field.new("FONDOS RENTA VARIABLE NACIONAL")
...
class MON::InvestmentFundsUSD < MON::AssetTable
	def load
		@name = "fondos de inversion internacional"
		@title = Field.new("FONDOS RENTA VARIABLE INTERNACIONAL")
		@table_end = [Field.new("Total"), Field.new("No registra posiciones")]
		@headers = []
			headers << HeaderField.new("Nemotécnico", headers.size, Setup::Type::LABEL, true)
			headers << HeaderField.new("Descripción", headers.size, Setup::Type::LABEL)
