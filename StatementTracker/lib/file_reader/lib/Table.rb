	...
	def set_range (line_size, line_height)
		unless @range
			...
			if @bottom
				...
				yf = line_height - 1 - @setup[3]
			end
			xi = @headers.first.left
			xf = @headers.last.right
			table_offset = @setup.offset
			xi = xi - table_offset >= @setup[0] ? xi - table_offset : @setup[0]
			xf = xf + table_offset <= line_size - @setup[1] ? xf + table_offset : line_size - @setup[1]
			@width = yf-yi+1
			@range = [xi,xf,yi,yf]
		end
	end
