...
class Reader
	...
	def initialize(context, file)
		...
		file_path = Dir["#{@file}/*_#{@page}.page"].first
		if file_path and File.exist? file_path
			first_page = File.new(file_path, 'r:UTF-8')
		end
		@bank = context
		@page_content = PageContent.new(@bank, @page, first_page.read)
	end
	...
	def read_next_field(field, from = nil, to = nil, verbose = false, inverted = false)
		...
		if not @page_content or @page_content.number != @page
			...
			@page_content = PageContent.new(@bank, @page, page.read)
		end
	...
	def mock_content content
		@page_content = PageContent.new(@bank, 0,content)
	end
	...
	def find_text text, limit = 0, with_offset = false
		...
		file_name = "#{@file[@file.rindex('/')+1..-1]}"
		counter = limit != 0 ? 1 : 0
		while (file_path = Dir["#{@file}/*_#{@page}.page"].first) and counter <= limit
			first_page = File.new(file_path, 'r:UTF-8')
			@page_content = PageContent.new(@bank, @page, first_page.read)
