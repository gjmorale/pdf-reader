class PageContent
	...
	def initialize(context, page_number, content)
		@number = page_number
		@content = content
	...
	def search_next(field, offset, from = nil, to = nil)
		...
		@content.lines[offset..@content.lines.size-field.width].each.with_index do |line, y_full|
			...
			line[from..to].match(field.regex){|m|
				...
				xf = (from + m.offset(0)[1] + field.text.length*@setup.text_expand).to_i
	...
	def horizontal_search(result, range, regex)
		...
		detected = false
		limit = range[1]
		while tolerance <= @setup.horizontal_search_range and range[1] + counter < line_size
			counter += 1
		...
		@content = @content.each_line.with_index.map{|line, i|
			if i >= range[2] and i <= range[3]
				...
				if stripped.match regex
					line[range[0]..range[1]] = @setup.wildchar*(range[1]-range[0])
				end
			end
			line
			}.join
	...
	def search_results_left(range, row, result)
		...
		counter = min 
		counter -= @setup.safe_zone if counter > @setup.safe_zone
		...
		while tolerance <= @setup.vertical_search_range and counter > range[0] 
			counter -= 1
